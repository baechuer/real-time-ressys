...
// Envelope is the success envelope used across event-service:
// {"data": ...}
// For errors we return auth-service style: {"error": {...}} directly
// (not nested under Envelope), so middleware/Err uses ErrorBody.
type Envelope struct {
	Data any `json:"data,omitempty"`
}

// ErrorBody matches auth-service error format.
...
// Fail writes auth-service style error body:
