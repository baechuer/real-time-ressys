...
func TestCalculateActionPolicy(t *testing.T) {
	...
	t.Run("Admin Permissions", func(t *testing.T) {
		...
		// Create a copy of futureEvent with Status=published
		publishedEvent := *futureEvent
		publishedEvent.Status = EventStatusPublished

		policy := CalculateActionPolicy(&publishedEvent, nil, otherUserID, "admin", now, false)
		assert.True(t, policy.CanCancelEvent)
		assert.True(t, policy.CanUnpublish)
	})
}
